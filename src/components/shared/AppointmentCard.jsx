...
const AppointmentCard = ({ appointment, onCancelRequest }) => {
    ...
    <>
      <div
        className="flex flex-col card gap-2 my-4 shadow-lg p-4 rounded-md"
        key={appointment.id}>
        <div className="flex justify-between">
          <div className="flex items-center gap-1">
            {isTeleVisit ? (
              <TeleVisitIcon className="blue" />
            ) : (
              <OfficeVisitIcon className="text-text-light-gray" />
            )}
        ...
        <p className="text-blue-medium font-medium text-base">
          {appointment.detail}
        </p>
        <div className="flex items-center gap-1">
          <AppointmentCalenderIcon width={14} height={16} />
          <time dateTime="" className="text-[#4B5563]">
            {appointment.date}
          </time>
          <TimeIcon className="ml-2" />
          <p className="text-[#4B5563]">{appointment.time}</p>
        </div>
        <div className="flex  flex-row justify-between gap-2">
          ...
          {appointment.name == 'checkIn' && (
            <div>
              <button className="px-4 py-1 bg-blue text-white text-base cursor-pointer rounded-full">
                Check In
              </button>
            </div>
          )}
