...
import MessageIcon from '../../assets/icons/svgs/Message';
...
const AppointmentMessagesSection = () => {
      ...
      <div
        ...
        <div className="inline-flex p-3 rounded bg-[var(--color-gray-50)] rounded-full">
          {title == 'Message Provider' ? (
            <MessageIcon className="text-blue-medium" />
          ) : (
    ...
    return (
      <article className="flex flex-col md:flex-row justify-between space-y-6">
        ...
        <div className="flex flex-col pl-10 pb-2 md:pl-0 md:pb-0">
          ...
          <p className="text-red-500 text-base text-appointment-cancel font-medium">
            Cancel Request
          </p>
        </div>
      </article>
  ...
  return (
    <>
      <RequestAppointment
        open={requestedAppointments}
        onClose={() => setRequestedAppointments(false)}
      />
      <CreateMessagePopup
        open={createMessage}
      ...
      <div className="flex flex-col lg:flex-row justify-between gap-4 space-x-4 p-2">
        <CardSection
          ...
          detail="Schedule your next appointment"
        />
        <CardSection
          title="Message Provider"
          detail="Get in touch with your provider"
        />
      </div>
    </>
  );
