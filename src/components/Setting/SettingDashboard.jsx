...
const SettingDashboard = () => {
    ...
    return (
      <div className={padding}>
        <p className="font-bold bg-[#F9FAFB] py-2 text-[#4B5563]">
  ...
  return (
    <>
      {isRepresenterRemove && (
        <Toast
          type={'Warning'}
          message={'Are you sure you want to delete representer?'}
          onClose={() => setRepresenterRemove(false)}
        />
      )}

      <form
        onSubmit={handleSubmit(updateDemographic)}
        className="flex flex-col bg-white m-4 rounded-lg shadow-sm">
        <div className="flex justify-between items-center p-4">
          <h3 className="text-base md:text-lg text-black font-medium">
            Demographics
          </h3>
          <button
            type="submit"
            className="text-white bg-blue rounded-full py-2 px-4 cursor-pointer text-sm  md:text-base">
            Save Changes
          </button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 p-4">
          <div>
            <TextInput
              ...
              inputClassName="border-border! h-10! bg-gray-100! cursor-not-allowed!"
            />
          </div>
          <div>
            <TextInput
              ...
              label={'Full Name'}
              labelClassName={'mb-1! text-base! font-normal'}
              inputClassName="border-[var(--color-border)]! h-10!"
            />
          </div>
          <div>
            <DatePickerInput
          ...
          <div>
            <TextInput
              ...
              label={'SSN'}
              labelClassName={'mb-0! text-base! font-normal'}
              inputClassName="border-border! h-10!"
            />
          </div>
          ...
          <div className="flex gap-3">
            ...
            <div className="flex-1">
              <SelectBox
                ...
                placeholder={false}
                labelClassName="mb-0! font-normal"
                SelectClassName="border-border! h-10!"
              />
            </div>
          </div>
          ...
          <div className="flex pt-0 md:pt-8">
            ...
            <span className="text-sm text-text-gray ml-5">E-Statement</span>
          </div>
        </div>
      </form>

      <Card title={'Insurance Plans'}>
        <div className="h-40 overflow-x-auto overflow-y-auto">
          {insurancesInfo.map((item, index) => (
            <div
              key={index}
              className="grid grid-flow-col auto-cols-fr min-w-max py-4">
      ...
      <Card title={'My Representatives'}>
        <div className="grid grid-cols-1 md:grid-cols-2 py-4 gap-4 px-4 gap-2">
          <div className="border border-border px-3 h-32 rounded-lg">
          ...
          <div className="border border-border p-2 rounded-lg">
            ...
            <div className="flex bg-gray-50 items-center rounded p-4">
              ...
              <input className="flex-1 border border-border mx-3 outline-none p-2 bg-white" />
              <button className="text-blue-medium text-sm cursor-pointer">
                Send Request
              </button>
            </div>
      ...
      <Card title={'Account Settings'}>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-14 p-4 items-center">
          ...
          <div className="pt-0 md:pt-6">
            <button
              type="submit"
              className="text-white bg-blue rounded-full py-2 px-4 cursor-pointer mt-2 sm:mt-0">
              Update Password
            </button>
          </div>
        </div>
