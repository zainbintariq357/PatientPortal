	...
	return (
		<>
			...
			<div className='flex flex-col min-h-screen bg-gray-50'>
				...
				<div className="flex bg-[var(--color-gray-100)] p-3 pl-6 gap-3">
					<button className={`${messageType == "active" ? 'bg-white' : ''} p-2 rounded-lg cursor-pointer flex items-center gap-1`} onClick={() => setMessageType("active")}>
						<MessagesIcon className={`${messageType == "active" ? 'text-[var(--color-blue-medium)]' : ''} inline-flex`} width={19.2} height={14.4} />
						<span className={`ml-1 ${messageType == "active" ? 'text-[var(--color-blue)]' : ''} font-medium text-sm md:text-base`}>Active Messages</span>
					</button>
					...
					<button className={`${messageType == "resolved" ? 'bg-white' : ''} p-2 rounded-lg cursor-pointer flex items-center gap-1`} onClick={() => setMessageType("resolved")}	>
						<MessagesIcon className={`${messageType == "resolved" ? 'text-[var(--color-blue-medium)]' : 'text-[var(--color-text-light-gray)]'} inline-flex`} width={19.2} height={14.4} />
						<span className={`ml-1 ${messageType == "resolved" ? 'text-[var(--color-blue)]' : ''}  font-medium text-sm md:text-base`}>Resolved Messages</span>
					</button>
				</div>
				...
				<div className="flex items-center justify-between mt-6 text-sm text-gray-60 mt-auto mb-6">
					...
					<div className="flex items-center gap-2 px-6">
						...
						<button className="px-3 py-1 rounded border border-gray-300 cursor-pointer bg-[var(--color-blue)] text-white">
							1
						</button>
